...
from django.views import View  # представления на основе класса
...
def index(request):
    return render(request, 'lecapp3/index.html')


def about(request):
    return render(request, 'lecapp3/about.html')


def author_posts(request, author_id):
    author = get_object_or_404(Author, pk=author_id)
    posts = Post.objects.filter(author=author).order_by('-id')[:5]
    return render(request, 'lecapp3/author_posts.html', {'author': author, 'posts': posts})


def post_full(request, post_id):
    post = get_object_or_404(Post, pk=post_id)
    return render(request, 'lecapp3/post_full.html', {'post': post})